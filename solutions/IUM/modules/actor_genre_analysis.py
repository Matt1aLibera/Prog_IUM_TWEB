...
def plot_actor_genre_composition(
    ...
    """
    # 19. Crea il grafico (commento originale)
    fig, ax1 = plt.subplots(figsize=figsize)

    # Barplot orizzontale con barre della stessa lunghezza (100%) (commento originale)
    bars = sns.barplot(x=[100] * len(top_actors), y=top_actors, color='lightgray', ax=ax1)

    # Colora le barre in base alla percentuale di genere (commento originale)
    ...
    # Aggiungi il numero totale di film come annotazione (commento originale)
    ...
    # Lineplot per la media degli anni di rilascio (commento originale)
    ax2 = ax1.twiny()
    ...
    # Aggiungi la scritta della media di rilascio (commento originale)
    ...
    # Aggiungi una linea verticale per l'anno medio globale (commento originale)
    ax2.axvline(global_avg_release_year + linegraph_offset, color='green',
                linestyle='--', linewidth=2,
                label='Media Anno di Rilascio (globale)')

    # Aggiungi un'annotazione per la media globale (commento originale)
    ...
    # Configurazione assi (commento originale)
    ax2.set_xlim(600 + linegraph_offset, 2050 + linegraph_offset)
    ax2.set_xticks([])
    ax1.set_xlim(0, 120)

    # Gestione legende (commento originale)
